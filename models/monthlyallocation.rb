...
class MonthlyAllocation
	...
	property :coeff,		Integer 						#@return [Integer] Coefficient for this allocation calculation. Set based on User's allocCoeff at time allocation created
	...
	# @param user [User] {User} to set or update MonthlyAllocation for
	...
	def self.setAllocationForUser(user, allocation, month=nil, year=nil)
		month = DateTime.now.month if(month.nil?)
		year = DateTime.now.year if(year.nil?)

		row = first(:uid => user.id, :month => month, :year => year, :order => [:id.desc])
		if(row.nil?)
			row = create(:uid => user.id, :allocation => allocation, :coeff => user.allocCoeff, :month => month, :year => year, :wasAutoSet => false)
		else
			row.update(:allocation => allocation, :coeff => user.allocCoeff, :wasAutoSet => false)
		end
	...
	# @param user [User] {User} to create an auto-set MonthlyAllocation for
	# @param allocation [Integer] % allocation (0-100)
	...
	def self.autoSetAllocationForUser(user, allocation, month=nil, year=nil)
		month = DateTime.now.month if(month.nil?)
		year = DateTime.now.year if(year.nil?)

		row = first(:uid => user.id, :month => month, :year => year, :order => [:id.desc])
		if(row.nil?)
			row = create(:uid => user.id, :allocation => allocation, :coeff => user.allocCoeff, :month => month, :year => year, :wasAutoSet => true)
		else
